...
describe('Building the test run specification', function()
	...
	describe('Running multiple roots', function()
		...
		it('Runs all tasks with matching roots', function()
			local expected = {
				{ command = { 'busted', '--output', output_handler_path, '--config-file', 'bustedrc', '--run', 'integration', '--', 'test/integration' } },
				{ command = { 'busted', '--output', output_handler_path, '--config-file', 'bustedrc', '--run', 'unit', '--', 'test/unit' } },
			}
