...
describe('Test file detection', function()
	...
	it('Rejects non-Lua unit files', function()
		...
		assert.is_false(adapter.is_test_file(fname), 'Test file should have been rejected')
	end)
	...
	it('Accepts unit test which matches the pattern', function()
		...
		for _, fname in ipairs(fnames) do
			assert.is_true(adapter.is_test_file(fname), 'Test file should have been accepted')
		end
	end)
	...
	it('Accepts E2E test with any name', function()
		local fnames = {'test/e2e/foo_spec.lua', 'test/e2e/foo.lua', 'test/e2e/foo/bar.lua'}
		for _, fname in ipairs(fnames) do
			assert.is_true(adapter.is_test_file(fname), 'Test file should have been accepted')
		end
	end)
	...
	it('Accepts non-literal pattern matches', function()
		local fnames = {'test/integration/test_foo.lua', 'test/integration/foo/test_bar.lua'}
		for _, fname in ipairs(fnames) do
			assert.is_true(adapter.is_test_file(fname), 'Test file should have been accepted')
		end
	end)
	...
	it('Rejects non-literal pattern mismatches', function()
		local fnames = {'test/integration/testing_foo.lua', 'test/integration/foo/testing_bar.lua'}
		for _, fname in ipairs(fnames) do
			assert.is_false(adapter.is_test_file(fname), 'Test file should have been rejected')
		end
	end)
